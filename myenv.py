...
import random
import torch
from mydata_util import read_data, random_get_dict
...
class Env:
    def __init__(self):
        ...
        self.min_datasize = config.sample_train_p
        self.frac = config.frac
        self.test_size = config.test_size
        self.per = config.per
        self.choose_number = config.frac * config.num_clients

        self.dataset_train, self.dataset_test, self.dataset_Q, self.dict_Q_train, self.dict_users, self.dict_users_test = read_data()
        self.dict_train_random = random_get_dict(self.dict_users, self.min_datasize)
        self.dict_test_random = random_get_dict(self.dict_users_test, self.min_datasize)
        self.data_size = np.array([len(self.dict_train_random[i]) for i in self.dict_train_random]).reshape(1, -1)
        ...
        self.idex=0
    def state_init(self):
        self.dict_train_random = random_get_dict(self.dict_users, self.min_datasize)  # 100-800
        self.data_size = np.array([len(self.dict_train_random[i]) for i in self.dict_train_random]).reshape(1, -1)
        self.data_quality = self.get_quality()
        ...
        self.r_n_e = np.random.randint(2048, 4096, size=(self.num_clients, self.num_servers))
        self.r_j_cloud = np.random.randint(4096, 8192, size=(1, self.num_servers))
        # self.net_glob_client = ResNet18_client_side()
        # self.net_glob_server = ResNet18_server_side(Baseblock, [2, 2, 2], 7)
        # self.net_model_server = [self.net_glob_server for i in range(int(self.num_clients))]
    ...
    def step(self, action, select_num):
        ...
        # print("get acc of global =  and the time = {}, energy = {}".format( total_time, sum_train_energy))
        # acc = batch_acc_list[len(batch_acc_list)-1]
        # self.acc_list.append(float(global_acc))
        # reward = (0.1 * global_acc) - (0.3*total_time) - (0.7*sum_train_energy)  # 注意参数
        reward = - (0.3*total_time) - (0.7*sum_train_energy)
        self.idex += 1
        ...
        # 接下来进行 state trans state' 变化范围：f r 和 q
        ...
        self.f_n = np.clip(self.f_n * ((100 - (10 - np.random.rand() * 20)) / 100), 1, 2)
        self.f_e = np.clip(self.f_e * ((100 - (10 - np.random.rand() * 20)) / 100), 2,5)
        self.r_n_e = np.clip(self.r_n_e * ((100 - (10 - np.random.rand() * 20)) / 100),2046,4096)
        self.r_j_cloud = np.clip(self.r_j_cloud * ((100 - (10 - np.random.rand() * 20)) / 100),4096,8192)
    ...
    def get_quality(self):
        ...
        for key, dict_set in self.dict_users_Q_test.items():
            # 随机采样
