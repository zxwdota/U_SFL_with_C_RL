...
def plot_kclusters(X, labels, centroids):
...
def greedy_merge_by_overlap_distance(X, labels, K, assign_k, center, margin=0.1, dist_threshold=0.8):
...
class NSKMeans:
    ...
    def fit(self, X, K, assign_k):
        ...
        for iter_num in range(self.K-1):
        ...
        for i in range(self.n_clusters-1):
            for j in range(i + 1, self.n_clusters-1):
                ...
                if bounds[i] is None or bounds[j] is None:
                    continue
                min_bound = np.maximum(bounds[i][0], bounds[j][0]) - appendbound
                max_bound = np.minimum(bounds[i][1], bounds[j][1]) + appendbound

                mask = np.all((pts >= min_bound) & (pts <= max_bound), axis=1)
                overlap_pts = pts[mask]
                pts_i_mask = ~np.any(np.all(pts_i[:, None, :] == overlap_pts[None, :, :], axis=2), axis=1)
                pts_j_mask = ~np.any(np.all(pts_j[:, None, :] == overlap_pts[None, :, :], axis=2), axis=1)

                pts_i_wo = pts_i[pts_i_mask]
                pts_j_wo = pts_j[pts_j_mask]
                for pt in overlap_pts:
                    # 模拟将 pt 加入 i 后的密度
                    if pts_j_wo.shape[0]==0:
                        pts_j_wo = pts_j
                    if pts_i_wo.shape[0]==0:
                        pts_i_wo = pts_i
                    bound_i_wo = [np.min(pts_i_wo, axis=0), np.max(pts_i_wo, axis=0)]
                    vol_i = np.prod(bound_i_wo[1] - bound_i_wo[0] + 1e-8)
                    rho_i_old = len(pts_i) / vol_i
                    pts_i_plus = np.vstack([pts_i, pt])
                    bound_i_wo_plus = [np.min(pts_i_plus, axis=0), np.max(pts_i_plus, axis=0)]
                    vol_i_plus = np.prod(bound_i_wo_plus[1] - bound_i_wo_plus[0] + 1e-8)
                    rho_i_new = len(pts_i_plus) / vol_i_plus

                    # 模拟将 pt 加入 j 后的密度
                    bound_j_wo = [np.min(pts_j_wo, axis=0), np.max(pts_j_wo, axis=0)]
                    vol_j = np.prod(bound_j_wo[1] - bound_j_wo[0] + 1e-8)
                    rho_j_old = len(pts_j) / vol_j
                    pts_j_plus = np.vstack([pts_j, pt])
                    bound_j_wo_plus = [np.min(pts_j_plus, axis=0), np.max(pts_j_plus, axis=0)]
                    vol_j_plus = np.prod(bound_j_wo_plus[1] - bound_j_wo_plus[0] + 1e-8)
                    rho_j_new = len(pts_j_plus) / vol_j_plus
                    to_i = rho_i_new+rho_j_old
                    to_j = rho_i_old+rho_j_new
                    # 归属到密度更大的那个簇
                    self.labels_[np.all(X == pt, axis=1)] = i if to_i > to_j else j

        self.labels_, self.cluster_centers_ = greedy_merge_by_overlap_distance(
            X, self.labels_, K=self.K, assign_k=self.assign_k,center=self.cluster_centers_, margin=0.2, dist_threshold=0.1
        )
            ...
            for i in range(self.n_clusters):
                for j in range(i + 1, self.n_clusters):
                    merged_once, self.labels_, self.cluster_centers_ = try_merge_clusters(
                        i, j, X, self.labels_, self.cluster_centers_, margin=0.2, dist_threshold=0.1
                    )
