    ...
    def __init__(self, block, num_blocks, num_classes=7):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7,
                               stride=2, padding=3, bias=False),                # conv1
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer2 = self._make_layer(block, 64, num_blocks[0], stride=1)       # conv2_x
        self.layer3 = self._make_layer(block, 128, num_blocks[1], stride=2)      # conv3_x
        self.layer4 = self._make_layer(block, 256, num_blocks[2], stride=2)      # conv4_x
        self.layer5 = self._make_layer(block, 512, num_blocks[3], stride=2)      # conv5_x
