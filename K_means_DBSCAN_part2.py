...
label_list = ['data_quality_stan', 'data_quantity_stan']
label_xlist = ['x1', 'x2']
X = clients_df[label_list]
...
PCA2_plot(X, y_dbscan)

silhouette_avg_dbscan, calinski_harabasz_dbscan, davies_bouldin_dbscan = safe_scores(X, y_dbscan)
...
for cluster_id in sorted(set(y_dbscan)):
    cluster_df = clients_df[y_dbscan == cluster_id].copy()
    filename = f"response_data/dbscan_cluster_{cluster_id}.pkl" if cluster_id != -1 else "response_data/dbscan_cluster_noise.pkl"
    with open(filename, 'wb') as f:
        pickle.dump(cluster_df, f)

kmeans = KMeans(n_clusters=3, random_state=config.seed).fit(X)
...
for cluster_id in sorted(set(y_kmeans)):
    cluster_df = clients_df[y_kmeans == cluster_id].copy()
    filename = f"response_data/kmeans_cluster_{cluster_id}.pkl" if cluster_id != -1 else "response_data/kmeans_cluster_noise.pkl"
    with open(filename, 'wb') as f:
        pickle.dump(cluster_df, f)