...
#                 if batch_idx == len(self.ldr_train)-1:
#                     finish = True
#                 client_output_grad = server.train_server(client_output_c, labels, choose_j_client_len, finish, batch_idx)
...
class Client:
    def __init__(self, cid, dataset, idxs, global_client_model, lr):
        self.cid = cid
        self.data = DataLoader(DatasetSplit(dataset, idxs),
                               batch_size=64, shuffle=True)
        self.model_C = copy.deepcopy(global_client_model).to(config.device)
        self.opt_C   = torch.optim.Adam(self.model_C.parameters(), lr=lr)

    def train_one_round(self, server_model):
        """server_model 已是本轮‘初始化’的 deepcopy"""
        self.model_C.train()
        server_model.train()
        opt_S = torch.optim.Adam(server_model.parameters(), lr=lr)
        ...
        for images, labels in self.data:
            ...
            loss_S = F.cross_entropy(out, labels)
            loss_S.backward()
            opt_S.step()
...
class Server:
    def __init__(self, sid, global_server_model, clients):
        self.sid = sid
        self.global_S = copy.deepcopy(global_server_model).to(device)
        self.clients  = clients          # list[Client]

    def train_one_round(self):
        """遍历本服务器下的所有客户端 -> intra-server 聚合"""
        client_pool, temp_S_pool = [], []

        for c in self.clients:
            # 为该客户端复制“当轮初始”服务器模型
            temp_S = copy.deepcopy(self.global_S).to(device)
            w_c, w_s = c.train_one_round(temp_S)
            client_pool.append(w_c)
            temp_S_pool.append(w_s)

        # ---- intra-server FedAvg ----
        new_client_state = average_weights(client_pool)
        new_server_state = average_weights(temp_S_pool)
...
#         self.model_list = []
#
#     def train_server(self, client_output_c, y, choose_j_client_len, finish, batch_idx):
#         if batch_idx == 0:
#             self.temp = copy.deepcopy(self.model_S).to(config.device_fl)
#         self.temp.train()
#         optimizer = torch.optim.Adam(self.temp.parameters(), lr=self.lr)
...
#         # with torch.no_grad():
#         #     print("Before:", self.temp.layer3[0].weight.view(-1)[0].item())
#         optimizer.step()
#         # with torch.no_grad():
#         #     print("After:", self.temp.layer3[0].weight.view(-1)[0].item())
#         if finish:
#             self.model_list.append(copy.deepcopy(self.temp))
#         if len(self.model_list) == choose_j_client_len:
#             # 进行模型聚合
#             global_model = local_fed_server(self.model_S, self.model_list)
...
def average_weights(state_dicts):
    ...
    for k in avg:
        for s in state_dicts[1:]:
            avg[k] += s[k]
        avg[k] = (avg[k] / len(state_dicts)).cpu()     # 放回 CPU
    return avg
...
# np.save(f'response_data/FL_data/50client_non_iid_v2/Q_loss_client{config.num_clients}.npy', np.array(q_loss))
...
for r in range(20):
    ...
    for srv in server_list:
        new_server_state = srv.train_one_round()
    ...
    # 把聚合后的 global_server_model 广播给所有服务器
    for srv in server_list:
        srv.global_S.load_state_dict(inter_server_state)

    # ======= 评估（示例）=======
    test_acc = evaluate(global_client_model, global_server_model, test_loader)
    print(f"Round {r:03d} | global test acc = {test_acc:.2f} %")