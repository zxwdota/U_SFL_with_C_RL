...
with open('return_0i_3.pkl', 'rb') as f:
    haed_split_return = -np.array(pickle.load(f))[0:4999]/50
with open('return_5i_1.pkl', 'rb') as f:
...
def annotate_stats(y, mv,x, std, label, color):
    ...
    plt.axhline(
        y=last_avg,
        color='black', linestyle='dashed', linewidth=1.5, alpha=0.7
    )
...
plt.plot(y_mv, old_split_return_mv, label='Origin server set', color=colors[1], linewidth=3)
plt.plot(y_mv, haed_split_return_mv, label='Less server set', color=colors[2])
...
plt.fill_between(y_mv, haed_split_return_mv - fix_std, haed_split_return_mv + fix_std,
                 alpha=0.3, color=colors[2],hatch='///'
)
...
# )
# 添加统计注释
annotate_stats(y_mv, haed_split_return_mv, haed_split_return, fix_std, label='head', color=colors[2])
