...
class PPO:
    ...
    def take_action(self, state,stepj,env):
        ...
        self.state_rms.update(state)  # <-- 先更新统计
        norm_state = self.state_rms.normalize(state)
        b_logic, old_mean_f, old_std_f = self.actor(norm_state.to(self.device))
    ...
    def update(self, transition_dict, client_len):
        ...
        self.state_rms.update(states)  # <-- 先更新统计
        ...
        actions_b = actions[:,0].to(int).reshape(config.num_clients, 1)
        actions_f = (actions[:,1]).reshape(config.num_clients, 1)

        orgin_states = torch.tensor(np.array(transition_dict['states']),
                              dtype=torch.float32).to(self.device)
        bb,mu,st = self.actor(norm_states)

        masks = [mask_actions(10, orgin_states[i]) for i in range(config.num_clients)]
        ...
        old_log_probs_f = dist.log_prob(actions_f/orgin_states[:,3].reshape(config.num_clients,1)).detach()
        ...
        for _ in range(self.epochs):
            ...
            log_probs_f = dist.log_prob(actions_f/orgin_states[:,3].reshape(config.num_clients,1))
            ...
            critic_loss = torch.mean(
                F.mse_loss(self.critic(norm_states), td_target.detach()))
        ...
        # if quit_bool:
        #     progress = self.global_step / config.num_episodes
        #     self.global_step += 1
        #     new_actor_lr = self.actor_lr * (1 - progress)
    ...
    for i in range(10):
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:  # 作用应该是将所有eposide分成10 turn，用tqdm来表示进度
            for i_episode in range(int(num_episodes / 10)):
                ...
                if config.server_quit:
                    if i==0 and i_episode==0:
                        server_quit_bool = True
                        if server_quit_bool:
                            # for param_group in agent.actor_optimizer.param_groups:
                            #     param_group['lr'] *= 0.05
                            # for param_group in agent.critic_optimizer.param_groups:
                            #     param_group['lr'] *= 0.05
                            print("服务器退出，调整学习率加速恢复")
                            # progress = agent.global_step / config.num_episodes
                            # agent.global_step += 1
                            # new_actor_lr = agent.actor_lr * (1 - progress)
                            # for param_group in agent.actor_optimizer.param_groups:
                            #     param_group['lr'] = new_actor_lr
                    if i==5 and i_episode==0:
                        for param_group in agent.actor_optimizer.param_groups:
                            param_group['lr'] *= 0.05
                        for param_group in agent.critic_optimizer.param_groups:
                            param_group['lr'] *= 0.05
                ...
                actor_loss, critic_loss = agent.update(transition_dict, len(env.server_data['location_x']))
...
import pickle
with open(f'response_data/clientserverquit/return_0i_3.pkl','wb') as f:
    pickle.dump(return_list,f)
with open(f'response_data/clientserverquit/time_0i_3.pkl','wb') as f:
    pickle.dump(t_list,f)
with open(f'response_data/clientserverquit/error_0i_3.pkl', 'wb') as f:
